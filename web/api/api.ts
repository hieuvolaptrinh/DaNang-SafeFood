const apiURL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080/api'; console.log('DEBUG: apiURL is', apiURL);
...
export const coSoKinhDoanhApi = {
  getList(params: { trangThai?: string; maPX?: string; page?: number; size?: number } = {}): Promise<CoSoKinhDoanhPageResponse> {
  ...
  updateDangKy(id: string, body: { tenCoSo: string; soGiayPhep: string; ngayHetHanGiayPhep: string; trangThai: string; maPX: string }): Promise<CoSoKinhDoanhItem> {
  ...
  kiemTra(id: string, body: { noiDung: string; maNguoiPhuTrach: string }): Promise<unknown> {
    return api.post(CSKD_BASE + '/' + id + '/kiemtra', body);
  },
  getDropdown(params: { keyword?: string; trangThai?: string } = {}): Promise<CoSoKinhDoanhItem[]> {
    const qs = new URLSearchParams();
...
export const giayChungNhanApi = {
  ...
  getList(params: { trangThai?: string; page?: number; size?: number } = {}): Promise<GiayChungNhanPageResponse> {
    const qs = new URLSearchParams();
    if (params.trangThai) qs.append('trangThai', params.trangThai);
    qs.append('page', String(params.page ?? 0));
    qs.append('size', String(params.size ?? 20));
...
export const thanhTraApi = {
  /** GET /api/v1/thanhtra */
  getList(params: { trangThai?: string; page?: number; size?: number } = {}): Promise<ThanhTraPageResponse> {
    const qs = new URLSearchParams();
    if (params.trangThai) qs.append('trangThai', params.trangThai);
    qs.append('page', String(params.page ?? 0));
    qs.append('size', String(params.size ?? 20));
