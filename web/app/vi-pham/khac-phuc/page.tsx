      ...
      <div className="max-w-[1200px] mx-auto px-6 py-8">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <h1 className="text-[28px] font-black text-slate-900 tracking-tight leading-tight">Theo dõi Khắc phục Vi phạm</h1>
            <p className="text-[13px] text-slate-400 mt-1 font-medium">Theo dõi tiến độ khắc phục vi phạm của các cơ sở kinh doanh tại Đà Nẵng</p>
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 mb-4 shadow-sm border border-slate-100">
          <p className="text-[12px] font-bold text-slate-400 uppercase tracking-wider mb-3">Tỷ lệ khắc phục theo mức độ vi phạm</p>
          ...
          <div className="flex gap-5 mt-2.5">
            {[
              ...
              <div key={item.label} className="flex items-center gap-1.5">
                ...
                <span className="text-[12px] font-bold text-slate-700">{item.val} hồ sơ</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3">
              <h2 className="text-[15px] font-bold text-slate-800">Tất cả yêu cầu khắc phục</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                {['Mã vi phạm', 'Tên cơ sở', 'Loại vi phạm', 'Mức độ', 'Trạng thái', 'Hạn khắc phục', 'Ngày cập nhật', 'Thao tác'].map((h) => (
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.length === 0 ? (
                <tr><td colSpan={8} className="px-5 py-12 text-center text-[13px] text-slate-400">Không tìm thấy hồ sơ khắc phục nào</td></tr>
              ) : filtered.map((v) => {
          ...
          <div className="px-5 py-3.5 border-t border-slate-100 flex items-center justify-between bg-slate-50/50">
            <span className="text-[12px] text-slate-400 font-medium">
              Hiển thị <strong className="text-slate-600">{filtered.length}</strong> trong tổng số <strong className="text-slate-600">{mockViolationFixes.length}</strong> hồ sơ
            </span>
