...
const PROCESSING_CONFIG: Record<string, { bg: string; text: string; icon: string }> = {
  'Đang xử lý': { bg: 'bg-amber-50', text: 'text-amber-700', icon: '🔄' },
  'Đã hoàn tất': { bg: 'bg-emerald-50', text: 'text-emerald-700', icon: '✓' },
};
...
function ProcessingBadge({ status }: { status: string }) {
  const cfg = PROCESSING_CONFIG[status] || { bg: 'bg-slate-50', text: 'text-slate-600', icon: '•' };
  return (
...
const STATS = [
  { label: 'Tổng hồ sơ', value: '438', icon: '📁', color: 'from-violet-600 to-purple-600' },
  { label: 'Nghiêm trọng', value: '87', icon: '🚨', color: 'from-red-500 to-rose-600' },
  { label: 'Trung bình', value: '156', icon: '⚠️', color: 'from-amber-500 to-orange-500' },
  { label: 'Đã hoàn tất', value: '312', icon: '✅', color: 'from-emerald-500 to-teal-500' },
];
  ...
  return (
    <div className="min-h-screen bg-[#f5f6fa] font-sans">
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-8">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="text-[13px] text-slate-400 mt-1 font-medium">
              Theo dõi tình trạng pháp lý và xử lý vi phạm của các cơ sở kinh doanh
            </p>
          </div>
        ...
        {/* Violation breakdown mini chart */}
        <div className="bg-white rounded-2xl p-5 mb-4 shadow-sm border border-slate-100">
          ...
          <div className="flex gap-2 h-2 rounded-full overflow-hidden">
            <div className="bg-red-500 rounded-full" style={{ flex: 87 }} title="Nghiêm trọng" />
            <div className="bg-amber-400 rounded-full" style={{ flex: 156 }} title="Trung bình" />
            <div className="bg-sky-400 rounded-full" style={{ flex: 195 }} title="Nhẹ" />
          </div>
          <div className="flex gap-5 mt-2.5">
            {[
              { color: 'bg-red-500', label: 'Nghiêm trọng', val: '20%' },
              { color: 'bg-amber-400', label: 'Trung bình', val: '36%' },
              { color: 'bg-sky-400', label: 'Nhẹ', val: '44%' },
            ].map((item) => (
              <div key={item.label} className="flex items-center gap-1.5">
                <span className={`w-2 h-2 rounded-full ${item.color}`} />
                <span className="text-[12px] text-slate-500 font-medium">{item.label}</span>
                <span className="text-[12px] font-bold text-slate-700">{item.val}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <h2 className="text-[15px] font-bold text-slate-800">Tất cả hồ sơ pháp lý</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                {['Mã cơ sở', 'Tên cơ sở', 'Loại vi phạm', 'Mức độ', 'Trạng thái xử lý', 'Ngày cập nhật', 'Thao tác'].map((h) => (
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.map((l) => (
                <tr key={l.id} className="hover:bg-violet-50/30 transition-colors group">
                  ...
                  <td className="px-5 py-3.5 text-[13px] text-slate-600 max-w-[200px]">
                    <span className="line-clamp-1">{l.violationType}</span>
                  </td>
                  <td className="px-5 py-3.5">
                    <SeverityBadge severity={l.severity} />
                  </td>
                  <td className="px-5 py-3.5">
                    <ProcessingBadge status={l.processingStatus} />
                  </td>
          ...
          <div className="px-5 py-3.5 border-t border-slate-100 flex items-center justify-between bg-slate-50/50">
            <span className="text-[12px] text-slate-400 font-medium">
              ...
              <strong className="text-slate-600">{mockLegalStatuses.length}</strong> hồ sơ
            </span>
